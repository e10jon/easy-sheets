...
const buildRange = (range: string, sheet?: string) =>
...
export default class EasySheets {
  ...
  public constructor(spreadsheetId: string, creds64: string) {
    ...
    this.serviceAccountCreds = JSON.parse(
      Buffer.from(creds64, "base64").toString()
  ...
  public getRange = async (
    ...
    if (opts.headerRow && values) {
      const headerKeys =
        opts.headerRow === "raw" ? values[0] : values[0].map(camelCase);
